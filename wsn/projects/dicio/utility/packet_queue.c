...
void push(packet_queue* pq, packet* p) {
	if(pq->size < MAX_PACKET_BUFFER) {
		...
		pq->buffer[pq->front].seq_num 		= p->seq_num;
		pq->buffer[pq->front].num_hops 		= p->num_hops;
		pq->buffer[pq->front].light_value 	= p->light_value;
		pq->buffer[pq->front].sensor_sample_rate 		= p->sensor_sample_rate;
		pq->buffer[pq->front].neighbor_update_rate 		= p->neighbor_update_rate;
	}
	return;
}
