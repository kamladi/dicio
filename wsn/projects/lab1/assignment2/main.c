...
void int_to_binary(int8_t rssi, int8_t *result);
...
void display_rssi(int8_t rssi) {
    int8_t led_array[4];
    uint8_t rssi_scaled = rssi / 5.25;
    if (rssi_scaled > 15) {
        rssi_scaled = 15;
    }
    printf("RSSI_SCALED=%d\n", rssi_scaled);

    int_to_binary(rssi_scaled, led_array);
...
void int_to_binary(int8_t rssi_scaled, int8_t *result) {
    for(int8_t i=3; i>=0; i--) {
        result[i] = rssi_scaled % 2;
        rssi_scaled = rssi_scaled / 2;
    }
    printf("binary: [");
    for(int i=0; i<4; i++)
        printf("%d", result[i]);
